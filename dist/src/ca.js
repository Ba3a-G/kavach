...
export class CA {
    ...
    generateAadhaarCertificate(csrData, userPublicKeyBytes) {
        ...
        const signature = Buffer.from(ed25519.sign(new Uint8Array(Buffer.from(JSON.stringify(certificateData))), this.privateKeyBytes)).toString('base64');
    ...
    static verifyAadhaarCertificate(certificate, govPublicKey) {
        ...
        return ed25519.verify(signatureRaw, new Uint8Array(Buffer.from(certDataString)), Signer.parsePublicKeyPEM(govPublicKey));
    }
    ;
    encodeAadhaarCertificateToPEM(certificate) {
    ...
    decodeAadhaarCertificateFromPEM(pemString) {
        ...
        return JSON.parse(certBuffer.toString('utf-8'));
    }
}
