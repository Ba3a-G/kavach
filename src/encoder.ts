...
export class Encoder {
  ...
  public decodeData(data: ArrayBuffer): {
    ...
    let version = Encoder.decodeVersion(bitString.slice(0, 3) as EncodedVersion);
    let gender = Encoder.decodeGender(bitString.slice(3, 5) as EncodedGender);
    let aadhaar = Encoder.decodeAadhaar(bitString.slice(5, 45).split("") as EncodedAadhaar);
    let dob = Encoder.decodeDOB(bitString.slice(45, 63).split("") as EncodedDOB);
